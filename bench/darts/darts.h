...
template <class node_type_, class node_u_type_, class array_type_, class array_u_type_,
...
class DoubleArrayImpl {
  public:
    ...
    int build(size_t key_size, const key_type** key, const size_t* length = 0, const value_type* value = 0,
    ...
    int open(const char* file, const char* mode = "rb", size_t offset = 0, size_t size = 0) {
        ...
        if (size_ != std::fread(reinterpret_cast<unit_t*>(array_), sizeof(unit_t), size_, fp)) return -1;
        std::fclose(fp);
    ...
    int save(const char* file, const char* mode = "wb", size_t offset = 0) {
        ...
        std::FILE* fp = std::fopen(file, mode);
        if (!fp) return -1;
        if (size_ != std::fwrite(reinterpret_cast<unit_t*>(array_), sizeof(unit_t), size_, fp)) return -1;
        std::fclose(fp);
        return 0;
    }

#ifdef HAVE_ZLIB_H
    int gzopen(const char* file, const char* mode = "rb", size_t offset = 0, size_t size = 0) {
        std::FILE* fp = std::fopen(file, mode);
        if (!fp) return -1;
        clear();
        ...
        zlib::gzread(gzfp, reinterpret_cast<unit_t*>(array_), sizeof(unit_t) * size_);
        zlib::gzclose(gzfp);
        return 0;
    }
    ...
    int gzsave(const char* file, const char* mode = "wb", size_t offset = 0) {
        zlib::gzFile gzfp = zlib::gzopen(file, mode);
        if (!gzfp) return -1;
        zlib::gzwrite(gzfp, reinterpret_cast<unit_t*>(array_), sizeof(unit_t) * size_);
        zlib::gzclose(gzfp);
        return 0;
    }
#endif
    ...
    template <class T>
    inline void exactMatchSearch(const key_type* key, T& result, size_t len = 0, size_t node_pos = 0) const {
        result = exactMatchSearch<T>(key, len, node_pos);
        return;
    }

    template <class T>
    inline T exactMatchSearch(const key_type* key, size_t len = 0, size_t node_pos = 0) const {
        ...
        if (static_cast<array_u_type_>(b) == array_[p].check && n < 0) set_result(&result, -n - 1, len);

        return result;
    }

    template <class T>
    size_t commonPrefixSearch(const key_type* key, T* result, size_t result_len, size_t len = 0,
    ...
    value_type traverse(const key_type* key, size_t& node_pos, size_t& key_pos, size_t len = 0) const {
        ...
        p = b;
        array_type_ n = array_[p].base;
        if (static_cast<array_u_type_>(b) == array_[p].check && n < 0) return -n - 1;
  ...
  private:
    ...
    size_t resize(const size_t new_size) {
        ...
        used_ = _resize(used_, alloc_size_, new_size, static_cast<unsigned char>(0));
    ...
    size_t insert(const std::vector<node_t>& siblings) {
        ...
        next:
            ...
            if (alloc_size_ <= (begin + siblings[siblings.size() - 1].code))
                resize(static_cast<size_t>(alloc_size_ * _max(1.05, 1.0 * key_size_ / progress_)));
        ...
        if (1.0 * nonzero_num / (pos - next_check_pos_ + 1) >= 0.95) next_check_pos_ = pos;

        used_[begin] = 1;
        size_ = _max(size_, begin + static_cast<size_t>(siblings[siblings.size() - 1].code + 1));

        for (size_t i = 0; i < siblings.size(); ++i) array_[begin + siblings[i].code].check = begin;

        for (size_t i = 0; i < siblings.size(); ++i) {
            std::vector<node_t> new_siblings;

            if (!fetch(siblings[i], new_siblings)) {
                array_[begin + siblings[i].code].base = value_ ? static_cast<array_type_>(-value_[siblings[i].left] - 1)

...
class LabelVector {
  public:
    ...
    LabelVector(const std::vector<std::vector<label_t> >& labels_per_level, const level_t start_level = 0,
                level_t end_level = 0 /* non-inclusive */) {
        if (end_level == 0) end_level = labels_per_level.size();

        num_bytes_ = 1;
        for (level_t level = start_level; level < end_level; level++) num_bytes_ += labels_per_level[level].size();

        // Modified by Shunsuke Kanda (+16 is for avoiding heap overflow in SIMD)
        labels_ = std::make_unique<label_t[]>(num_bytes_ + 16);
  ...
  public:
    ...
    void save(std::ostream& os) const {
        saveValue(os, num_bytes_);
        saveArray(os, labels_, num_bytes_ + 16);  // +16 is for avoiding heap overflow in SIMD
    }
    void load(std::istream& is) {
        loadValue(is, num_bytes_);
        loadArray(is, labels_, num_bytes_ + 16);  // +16 is for avoiding heap overflow in SIMD
    }

  private:
    // Modified by Shunsuke Kanda
    position_t num_bytes_ = 0;
    std::unique_ptr<label_t[]> labels_;
    // label_t* labels_;
};
...
bool LabelVector::simdSearch(const label_t target, position_t& pos, const position_t search_len) const {
    ...
    while ((num_labels_left >> 4) > 0) {
        // Modified by Shunsuke Kanda
        label_t* start_ptr = labels_.get() + pos + num_labels_searched;
        // label_t* start_ptr = labels_ + pos + num_labels_searched;
        __m128i cmp = _mm_cmpeq_epi8(_mm_set1_epi8(target), _mm_loadu_si128(reinterpret_cast<__m128i*>(start_ptr)));

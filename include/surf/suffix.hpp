...
class BitvectorSuffix : public Bitvector {
  public:
    ...
    BitvectorSuffix(const SuffixType type, const level_t hash_suffix_len, const level_t real_suffix_len,
    ...
    static word_t constructRealSuffix(const std::string& key, const level_t level, const level_t len) {
    ...
    static word_t constructMixedSuffix(const std::string& key, const level_t hash_len, const level_t real_level,
    ...
    static word_t constructSuffix(const SuffixType type, const std::string& key, const level_t hash_len,
    ...
    position_t serializedSize() const {
        position_t size =
            sizeof(num_bits_) + sizeof(type_) + sizeof(hash_suffix_len_) + sizeof(real_suffix_len_) + bitsSize();
        sizeAlign(size);
        return size;
    }
  ...
  public:
    ...
    void save(std::ostream& os) const {
        Bitvector::save(os);
        saveValue(os, type_);
        saveValue(os, hash_suffix_len_);
        saveValue(os, real_suffix_len_);
    }
    void load(std::istream& is) {
        Bitvector::load(is);
        loadValue(is, type_);
        loadValue(is, hash_suffix_len_);
        loadValue(is, real_suffix_len_);
    }

  private:
    // Modified by Shunsuke Kanda
    SuffixType type_ = kNone;
    level_t hash_suffix_len_ = 0;  // in bits
    level_t real_suffix_len_ = 0;  // in bits
};
...
word_t BitvectorSuffix::read(const position_t idx) const {
    ...
    if (offset + suffix_len > kWordSize) ret_word += (bits_[word_id + 1] >> (kWordSize - offset - suffix_len));
    return ret_word;
}
...
int BitvectorSuffix::compare(const position_t idx, const std::string& key, const level_t level) const {
    if ((idx * getSuffixLen() >= num_bits_) || (type_ == kNone) || (type_ == kHash)) return kCouldBePositive;

    word_t stored_suffix = read(idx);
    word_t querying_suffix = constructRealSuffix(key, level, real_suffix_len_);
    if (type_ == kMixed) stored_suffix = extractRealSuffix(stored_suffix, real_suffix_len_);
